...
- 新成员入群后，机器人会删除入群提示并发送验证消息。
- 验证消息包含：欢迎文案、验证按钮、管理员操作按钮：
  - ✔️ 直接通过（跳过验证）
  - ❌ 踢出并加入黑名单（默认 30 天）
- 群组处理方式可在群内通过 `/group_setting` 配置（需启用且配置 Redis）：认证剔除、手动解封、无作为、静默 1/2 周等。
...
| `/group_setting` | 群组设置：新成员入群处理方式等（需启用并配置 Redis）。                                    |
...
镜像使用项目内 `docker/Dockerfile` 构建，入口为 `docker/startup.sh`（会创建 `log` 目录并执行 `python main.py`）。
...
构建使用 Python 3.12 Alpine，已包含 ffmpeg、nodejs 等运行时依赖。
...
| 成员验证码 | ✅ 正常 | 35 个测试通过 |
| `/chat` | 被注释 | 需要 Redis |
| `/group_setting` | 被注释 | 需要 Redis |
| `/translate` | 被注释 | 需要外部翻译 API |
| `/tts` | 被注释 | 需要 edge-tts 依赖 |
| `/asr` | 被注释 | 需要外部服务 |
| `/shorturl` | 被注释 | 需要外部 API |
...
确保 `main.ini` 中配置了 Redis DSN 并且服务正常运行。
