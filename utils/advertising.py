...
def _get_config_bool(section, key: str, default: bool = False) -> bool:
    ...
    Supports ConfigParser SectionProxy (with getboolean) and dict-like objects.
    """
    # Try ConfigParser's getboolean method first
    try:
        return section.getboolean(key, fallback=default)
    except AttributeError:
        # Fallback to dict-like get
...
def load_advertising_words() -> List[str]:
    ...
    # Support both ConfigParser and dict (for testing)
    advertising_section = config["advertising"]
    if not _get_config_bool(advertising_section, "enabled", False):
        logger.info("Advertising detection is disabled")
        return []

    words = advertising_section.get("words", "")
...
def load_advertising_patterns() -> List[Dict[str, Any]]:
    ...
    patterns_str = advertising_section.get("regex_patterns", "")
    if not patterns_str:
        return []
